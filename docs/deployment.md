...
- `EEVEE_DEMO_VIDEO_URL` and `EEVEE_REPO_URL` (optional): the landing page shows **Watch demo** and **GitHub** buttons only when these are set.
...
9. Record the demo against this deployed commit, then fill `<LIVE_URL>` and `<DEMO_VIDEO_URL>` in `README.md`.
