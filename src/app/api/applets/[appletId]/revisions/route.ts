...
import { reviseVersionSchema } from '@/domain/applet'
import { ensureWorkspace, reviseVersion } from '@/server/applets'
...
export async function POST(
  request: NextRequest,
  context: RouteContext<'/api/applets/[appletId]/revisions'>,
  ...
  try {
    ...
    const [{ appletId }, input] = await Promise.all([
      context.params,
      parseJson(request, reviseVersionSchema),
    ])
    const result = await reviseVersion(session.workspaceId, appletId, input)
