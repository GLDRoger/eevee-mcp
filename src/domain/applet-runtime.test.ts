...
describe('prepareAppletRuntime', () => {
  it('injects the isolated runtime before the compiled app and escapes script-breaking input', () => {
    ...
    expect(compiled).toContain('const evaluationToken = crypto.randomUUID()')
    expect(compiled).not.toMatch(/evaluationToken = "/)
    expect(compiled).toContain('native.querySelectorAll.call(document, selector)')
    expect(compiled).toContain("getter(HTMLElement.prototype, 'innerText')")
    ...
    expect(compiled).toContain("case 'settle':")
    expect(compiled).toContain('settleBridgeWork()')
