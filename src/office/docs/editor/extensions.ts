 ...
 * `generatedChanged` (diff highlighting for generated edits).
 */
...
const anchorAttrs = {
  ...
  generatedChanged: { default: false },
...
function blockAttrs(
  ...
  if (node.attrs.generatedChanged) classes.push('generated-changed')
...
export const DocInlineMath = Node.create({
  ...
  addAttributes() {
    return {
      ...
      /** flat token strip (word count / plain-text fallback) */
      text: { default: '' },
    }
  },
  parseHTML() {
      ...
      dom.addEventListener('dblclick', () => {
        ...
        if (latex && typeof pos === 'number') {
          window.dispatchEvent(
            new CustomEvent('eevee-docs-edit-inline-math', {
...
export const DocNestedTable = Node.create({
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      window.addEventListener('eevee-docs-commit-tables', commit)
      ...
      return {
        ...
        destroy: () => {
          window.removeEventListener('eevee-docs-commit-tables', commit)
 ...
 * the table (or when App broadcasts 'eevee-docs-commit-tables' before saving).
 */
function wireTableEditing(
  ...
  return {
    setEditable,
    commit,
    cleanup: () => window.removeEventListener('eevee-docs-commit-tables', commit),
  }
}
...
function mountTextboxEditors(
  ...
  /** external model change (undo of a commit, external edit): re-feed the sub-editors */
