...
export async function savePptxToFile(opened: OpenedPptx, filePath: string): Promise<void> {
  if (!isNodeRuntime()) {
    throw new Error('savePptxToFile requires Node.js; use savePptx in the browser')
  }
  // Specifiers stay dynamic so a browser bundler never follows node:fs / node:stream.
  const nodeFs = 'node:fs'
  const nodeStream = 'node:stream/promises'
  const { createWriteStream } = (await import(nodeFs)) as typeof import('node:fs')
  const { pipeline } = (await import(nodeStream)) as typeof import('node:stream/promises')
...
export function commitSaved(opened: OpenedPptx): void {
  ...
  for (const slide of deck.slides) {
    ...
    archive.entries.set(slide.path, utf8Encode(xml))
  }
}
...
/** JSZip's instanceof Uint8Array check fails across the jsdom/Node realm boundary. */
function zipContent(data: Uint8Array): Uint8Array {
...
function buildZip(opened: OpenedPptx): JSZip {
  ...
  for (const [path, data] of archive.entries) {
    ...
    zip.file(path, zipContent(data), COMPRESSED_EXTENSIONS.has(ext) ? {} : { compression: 'STORE' })
  }
  return zip
}
...
function registerNewSlide(opened: OpenedPptx, sourceIndex: number, newPath: string): Slide | null {
  ...
  if (ct) {
    ...
    archive.entries.set(ctPath, utf8Encode(ct.replace('</Types>', `${override}</Types>`)))
  }
  ...
  archive.entries.set(
    presRelsPath,
    utf8Encode(presRels.replace('</Relationships>', `${relXml}</Relationships>`)),
  )
  ...
  archive.entries.set(presPath, utf8Encode(nextPres))
...
export function duplicateSlide(
  ...
  archive.entries.set(newPath, utf8Encode(patchSlideXml(src)))

  const srcRels = archive.readText(relsPathFor(src.path))
  if (srcRels) {
    const cleaned = srcRels.replace(/<Relationship\s[^>]*\/notesSlide"[^>]*\/>/g, '')
    archive.entries.set(relsPathFor(newPath), utf8Encode(cleaned))
  }
...
export function pasteSlide(
  ...
  archive.entries.set(newPath, utf8Encode(bundle.slideXml))
  archive.entries.set(relsPathFor(newPath), utf8Encode(relsXml))
...
export function insertBlankSlide(opened: OpenedPptx, sourceIndex: number): Slide | null {
  ...
  const newPath = nextSlidePath(archive)
  archive.entries.set(newPath, utf8Encode(BLANK_SLIDE_XML))
  ...
  const rels =
    ...
    (layout
      ...
      : '') +
    '</Relationships>'
  archive.entries.set(relsPathFor(newPath), utf8Encode(rels))
...
function ensureDefaultContentType(archive: PackageArchive, ext: string, contentType: string): void {
  ...
  archive.entries.set(ctPath, utf8Encode(ct.slice(0, at) + def + ct.slice(at)))
}
...
export async function mergeSlideFromPptx(
  ...
  archive.entries.set(newPath, utf8Encode(slideXml))
  const relsXml =
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\r\n' +
    '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">' +
    newRelsLines.join('') +
...
export function deleteSlide(opened: OpenedPptx, index: number): boolean {
  ...
  archive.entries.set(presPath, utf8Encode(pres.replace(sldTag, '')))
  const relTag = new RegExp(`<Relationship\\s[^>]*Id="${rid}"[^>]*/>`).exec(presRels)?.[0]
  if (relTag) {
    archive.entries.set(presRelsPath, utf8Encode(presRels.replace(relTag, '')))
  }
  ...
  if (ct) {
    ...
    if (override) archive.entries.set(ctPath, utf8Encode(ct.replace(override, '')))
  }
...
export function materializeSlide(opened: OpenedPptx, slideIndex: number): Slide | null {
  ...
  if (!slide) return null
  archive.entries.set(slide.path, utf8Encode(patchSlideXml(slide)))
...
export function setSlideLayout(
  ...
  opened.archive.entries.set(relsPath, utf8Encode(next))
...
export function setSlideSize(opened: OpenedPptx, cx: number, cy: number): boolean {
  ...
  archive.entries.set(presPath, utf8Encode(next))
  deck.size = { cx, cy }
  ...
  for (const part of listMasterParts(archive)) {
    ...
    if (touched) archive.entries.set(part.partPath, utf8Encode(patchSlideXml(partSlide)))
  }
...
export function ensureTableStylePart(
  ...
  archive.entries.set(path, utf8Encode(next))
  if (existing) return
  const ctPath = '[Content_Types].xml'
  const ct = archive.readText(ctPath)
  ...
  if (presRels && !presRels.includes('/relationships/tableStyles"')) {
    ...
    archive.entries.set(
      presRelsPath,
      utf8Encode(presRels.replace('</Relationships>', rel + '</Relationships>')),
    )
  }
}
...
export function editChartElement(
  ...
  archive.entries.set(chartPath, utf8Encode(newXml))
  slide.structureDirty = true
  return true
}
...
export function pasteElements(
  ...
  if (relsDirty) archive.entries.set(relsPath, utf8Encode(relsXml))
