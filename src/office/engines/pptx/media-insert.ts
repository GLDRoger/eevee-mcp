...
import { concatBytes, utf8Encode, writeUint32BE } from './bytes'
...
function pngChunk(type: string, data: Uint8Array): Uint8Array {
  ...
  head.set(utf8Encode(type), 4)
  const crcBuf = new Uint8Array(4)
  writeUint32BE(crcBuf, 0, crc32(concatBytes([utf8Encode(type), data])))
...
function ensureDefaultContentType(opened: OpenedPptx, ext: string, mime: string): void {
  ...
  if (ct && !new RegExp(`<Default Extension="${ext}"`).test(ct)) {
    ...
    opened.archive.entries.set(ctPath, utf8Encode(ct.replace('</Types>', `${dflt}</Types>`)))
  }
}
...
function appendRels(
  ...
  opened.archive.entries.set(relsPath, utf8Encode(xml))
  return rids
}
