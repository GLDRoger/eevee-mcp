...
const child = (value: unknown, key: string): XmlNode => asXmlNode(asXmlNode(value)[key])
const attribute = (value: unknown, key: string): string | undefined => {
  const raw = asXmlNode(value)[key]
...
function parseBackground(xml: string, ctx: ParseContext): Fill | undefined {
  ...
  let doc: XmlNode
  try {
    doc = asXmlNode(parser.parse(m[0]))
  } catch {
    return undefined
  }
  const bg = child(doc, 'p:bg')
...
function parseShapeFragment(
  ...
  const semanticXml = fragXml
    ...
    .replace(/<\/a:fld>/g, '</a:r>')
  const doc = asXmlNode(parser.parse(semanticXml))
  const candidates = xmlArray(doc[sp.name])
...
function parseSpShape(
  ...
  const phRaw = child(nv, 'p:nvPr')['p:ph']
  const ph = asXmlNode(phRaw)
  const phType = attribute(ph, '@_type')
  const phIdx = attribute(ph, '@_idx')
  const name = attribute(child(nv, 'p:cNvPr'), '@_name')
  ...
  const presetGeometry = attribute(prstGeom, '@_prst')
  ...
  const styleRaw = node['p:style']
  if (styleRaw && typeof styleRaw === 'object') {
    const style = asXmlNode(styleRaw)
    if (fill === undefined) {
      const ref = child(style, 'a:fillRef')
      const idx = parseInt(attribute(ref, '@_idx') ?? '0', 10) || 0
    ...
    if (!shadow && !glow) {
      const ref = child(style, 'a:effectRef')
      const idx = parseInt(attribute(ref, '@_idx') ?? '0', 10) || 0
      const phClr = resolveColorNode(ref, ctx)
  ...
  const el: TextElement = {
    ...
    placeholder: phRaw != null ? (phType ?? 'body') : undefined,
    name,
    presetGeometry,
    ...(adjust ? { adjust } : {}),
...
function styleRefStroke(node: XmlNode, ctx: ParseContext): Stroke | undefined {
  const ref = child(child(node, 'p:style'), 'a:lnRef')
  const idx = parseInt(attribute(ref, '@_idx') ?? '0', 10) || 0
...
function parseStroke(
  ...
  const spPr = asXmlNode(spPrRaw)
  const lnRaw = spPr['a:ln']
  if (!lnRaw || typeof lnRaw !== 'object') return undefined
  ...
  const dash = attribute(child(ln, 'a:prstDash'), '@_val')
  const capValue = attribute(ln, '@_cap')
  const cap = capValue ? capMap[capValue] : undefined
...
function parseArrowEnd(value: unknown): ArrowEnd | undefined {
  ...
  return {
    type,
    ...(wRaw ? { w: sizeMap[String(wRaw)] ?? 'med' } : {}),
    ...(lenRaw ? { len: sizeMap[String(lenRaw)] ?? 'med' } : {}),
  }
}
 ...
 */
function parseConnector(node: XmlNode, anchor: ByteAnchor, ctx: ParseContext): TextElement {
  const spPr = child(node, 'p:spPr')
  const nvCxn = child(node, 'p:nvCxnSpPr')
  const name = attribute(child(nvCxn, 'p:cNvPr'), '@_name')
  const prstGeom = child(spPr, 'a:prstGeom')
  ...
  const cxnPr = child(nvCxn, 'p:cNvCxnSpPr')
  const st = cxnPr['a:stCxn']
  const end = cxnPr['a:endCxn']
  const cxnRef = (value: unknown) => {
    const id = attribute(value, '@_id')
    return id != null ? { id: parseInt(id, 10), idx: intOr(attribute(value, '@_idx'), 0) } : undefined
  }
  const connection =
    st || end
      ? {
  ...
  return {
    ...
    name,
    presetGeometry: attribute(prstGeom, '@_prst') ?? 'line',
...
function parseGlow(spPr: unknown, ctx: ParseContext): import('./types').GlowEffect | undefined {
  const glowRaw = child(spPr, 'a:effectLst')['a:glow']
  if (!glowRaw || typeof glowRaw !== 'object') return undefined
  const glow = asXmlNode(glowRaw)
  const color = resolveColorNode(glow, ctx)
  if (!color) return undefined
  const rad = intOr(glow['@_rad'], 0)
  return { color, radius: Number.isFinite(rad) ? rad : 0 }
}

function parseShadow(spPr: unknown, ctx: ParseContext): ShadowEffect | undefined {
  const shdwRaw = child(spPr, 'a:effectLst')['a:outerShdw']
  if (!shdwRaw || typeof shdwRaw !== 'object') return undefined
  const shdw = asXmlNode(shdwRaw)
...
function parseAvLst(value: unknown): Record<string, number> | undefined {
  const avLst = asXmlNode(value)
  const gdRaw = avLst['a:gd']
  if (!gdRaw) return undefined
  const list = xmlArray(gdRaw)
  const out: Record<string, number> = {}
  for (const gd of list) {
    const name = attribute(gd, '@_name')
    const m = /^val\s+(-?\d+)/.exec(attribute(gd, '@_fmla') ?? '')
...
function parseGroup(
  ...
  const xfrmRaw = grpSpPr['a:xfrm']
  const xfrm = asXmlNode(xfrmRaw)
  const transform = parseXfrm(xfrm)
  const name = attribute(child(child(node, 'p:nvGrpSpPr'), 'p:cNvPr'), '@_name')

  // Child coordinate system: <a:chOff>/<a:chExt> (child coords are based on it, mapped to the parent when rendering)
  const chOffRaw = xfrm['a:chOff']
  const chExtRaw = xfrm['a:chExt']
  const chOff = asXmlNode(chOffRaw)
  const chExt = asXmlNode(chExtRaw)
  const childOffset =
    chOffRaw || chExtRaw
      ? {
          x: chOffRaw ? intOr(chOff['@_x'], 0) : 0,
          y: chOffRaw ? intOr(chOff['@_y'], 0) : 0,
          cx: chExtRaw ? intOr(chExt['@_cx'], 0) : 0,
          cy: chExtRaw ? intOr(chExt['@_cy'], 0) : 0,
        }
      : undefined
...
function groupChildNvId(value: XmlNode): string | undefined {
  for (const key of ['p:nvSpPr', 'p:nvPicPr', 'p:nvGrpSpPr', 'p:nvGraphicFramePr', 'p:nvCxnSpPr']) {
    const id = child(child(value, key), 'p:cNvPr')['@_id']
...
function blipEmbedId(value: unknown): string | undefined {
  const blip = asXmlNode(value)
  const direct = attribute(blip, '@_r:embed')
  if (direct) return direct
  const exts = child(blip, 'a:extLst')['a:ext']
  for (const ext of xmlArray(exts)) {
    for (const [key, candidate] of Object.entries(ext)) {
      if (key === 'svgBlip' || key.endsWith(':svgBlip')) {
        const id = attribute(candidate, '@_r:embed')
        if (id) return id
      }
    }
  }
...
function parsePicture(node: XmlNode, anchor: ByteAnchor, ctx: ParseContext): PictureElement {
  const spPr = child(node, 'p:spPr')
  const transform = parseXfrm(spPr['a:xfrm'])
  const blipFill = child(node, 'p:blipFill')
  const blip = child(blipFill, 'a:blip')
  const embedId = blipEmbedId(blip)
  const mediaRef = (embedId && ctx.mediaRels?.get(embedId)) || ''
  const cNvPr = child(child(node, 'p:nvPicPr'), 'p:cNvPr')
  const name = attribute(cNvPr, '@_name')
  const descr = attribute(cNvPr, '@_descr')
  ...
  const prstGeom = child(spPr, 'a:prstGeom')
  const picGeom = attribute(prstGeom, '@_prst')
  const picAdjust = parseAvLst(prstGeom['a:avLst'])
  const softEdgeRad = attribute(child(child(spPr, 'a:effectLst'), 'a:softEdge'), '@_rad')
  const alphaAmt = attribute(child(blip, 'a:alphaModFix'), '@_amt')
  const opacity =
  ...
  const nvPr = child(child(node, 'p:nvPicPr'), 'p:nvPr')
  ...
  if (avNode !== undefined) {
    ...
    const link = attribute(avNode, '@_r:link')
...
function parseSrcRect(value: unknown): PictureElement['srcRect'] | undefined {
  if (!value || typeof value !== 'object') return undefined
  const sr = asXmlNode(value)
...
function graphicFramePassthrough(node: XmlNode, anchor: ByteAnchor, ctx: ParseContext): SlideElement {
  const data = child(child(node, 'a:graphic'), 'a:graphicData')
  const uri = attribute(data, '@_uri') ?? ''
  ...
  if (kind === 'smartart') {
    const dm = attribute(child(data, 'dgm:relIds'), '@_r:dm')
...
function parseDiagramDrawing(drawingXml: string, ctx: ParseContext): SlideElement[] {
  ...
  let doc: XmlNode
  try {
    doc = asXmlNode(parser.parse(xml))
  } catch {
    return []
  }
  const spTreeRaw = child(doc, 'p:drawing')['p:spTree']
  if (!spTreeRaw) return []
  const spTree = asXmlNode(spTreeRaw)
  const sps = xmlArray(spTree['p:sp'])
...
function findDescendantPic(value: unknown, depth = 0): XmlNode | undefined {
  if (!value || typeof value !== 'object' || depth > 6) return undefined
  const node = asXmlNode(value)
  const pics = node['p:pic']
  if (pics) return xmlArray(pics)[0]
...
function parseTable(
  ...
  const tbl = asXmlNode(tableValue)
  const gridRaw = child(tbl, 'a:tblGrid')['a:gridCol']
  const gridCols = xmlArray(gridRaw)
  const colWidths = gridCols.map((g) => intOr(g['@_w'], 0))
  const trsRaw = tbl['a:tr']
  const trs = xmlArray(trsRaw)
  ...
  const styleId =
    typeof styleIdRaw === 'string' ? styleIdRaw : attribute(styleIdRaw, '#text')
  ...
  const rows: TableCell[][] = trs.map((tr, r) => {
    ...
    const tcs = xmlArray(tcsRaw)
    const gridCols = tableRowGridCols(
      tcs.map((tc) => ({
        gridSpan: tc['@_gridSpan'] ? intOr(tc['@_gridSpan'], 1) : 1,
  ...
  return {
    ...
    anchor,
    transform: parseXfrm(node['p:xfrm']),
    name: attribute(child(child(node, 'p:nvGraphicFramePr'), 'p:cNvPr'), '@_name'),
    colWidths,
    rowHeights,
    rows,
...
function parseTableCell(
  ...
  if (tc['a:txBody']) {
    ...
    const anchor = attribute(tcPr, '@_anchor')
    if (anchor) text.anchor = anchorMap[anchor]
    text.insets = {
  ...
  const gridSpan = tc['@_gridSpan'] ? intOr(tc['@_gridSpan'], 0) : undefined
  const rowSpan = tc['@_rowSpan'] ? intOr(tc['@_rowSpan'], 0) : undefined
...
function passthrough(
  ...
  const spPr = asXmlNode(node['p:spPr'] ?? node['p:grpSpPr'])
  const transform = parseXfrm(spPr['a:xfrm'])
...
function parseXfrm(value: unknown): Transform {
  ...
  return {
    offset: {
      x: offRaw ? intOr(off['@_x'], 0) : 0,
      y: offRaw ? intOr(off['@_y'], 0) : 0,
      cx: extRaw ? intOr(ext['@_cx'], 0) : 0,
      cy: extRaw ? intOr(ext['@_cy'], 0) : 0,
    },
    rot: xfrm['@_rot'] ? intOr(xfrm['@_rot'], 0) : 0,
...
function parseFill(value: unknown, ctx: ParseContext): Fill | undefined {
  if (!value) return undefined
  const spPr = asXmlNode(value)
  ...
  if (blip) {
    const blipNode = asXmlNode(blip)
    const embedId = blipEmbedId(blipNode['a:blip'])
    const mediaRef = (embedId && ctx.mediaRels?.get(embedId)) || ''
    if (mediaRef)
      return { type: 'image', mediaRef, mode: 'a:tile' in blipNode ? 'tile' : 'stretch' }
  }

  const pat = spPr['a:pattFill']
  if (pat) {
    const patNode = asXmlNode(pat)
    const fg = resolveColorNode(patNode['a:fgClr'], ctx) ?? '#000000'
    const bg = resolveColorNode(patNode['a:bgClr'], ctx) ?? '#FFFFFF'
    return { type: 'pattern', fg, bg, preset: String(patNode['@_prst'] ?? 'pct50') }
  }
...
function parseGradFill(value: unknown, ctx: ParseContext): Fill | undefined {
  const grad = asXmlNode(value)
  const gsLst = child(grad, 'a:gsLst')['a:gs']
  const list = xmlArray(gsLst)
  const stops = list
    .map((gs) => {
      const pos = intOr(gs['@_pos'], 0) / 100000 // 0..100000 → 0..1
      const color = resolveColorNode(gs, ctx)
      return color ? { pos, color } : null
    })
    .filter((stop): stop is { pos: number; color: string } => stop != null)
  if (!stops.length) return undefined
  // Linear gradient angle: <a:lin ang=""> (unit 1/60000 degree); radial etc. get the default angle for now
  const ang = attribute(child(grad, 'a:lin'), '@_ang')
  const angle = ang != null ? parseInt(ang, 10) || 0 : undefined
  const pathType = attribute(child(grad, 'a:path'), '@_path')
  return {
    type: 'gradient',
    stops,
    ...(angle != null ? { angle } : {}),
...
function resolveColorNode(node: unknown, ctx: ParseContext): string | undefined {
  return resolveColorNodeShared(node, ctx.theme, ctx.phClr)
}

// ── Text ─────────────────────────────────────────────────────────────

function parseTextBody(value: unknown, ctx: ParseContext, phChain: TextStyleLevels[] = []): TextBody {
  const txBody = asXmlNode(value)
  const bodyPrRaw = txBody['a:bodyPr']
  const bodyPr = bodyPrRaw && typeof bodyPrRaw === 'object' ? bodyPrRaw : {}
  const anchorMap: Record<string, TextBody['anchor']> = { t: 'top', ctr: 'middle', b: 'bottom' }
  const paras = xmlArray(txBody['a:p'])
  ...
  const paragraphs: Paragraph[] = paras.map((paragraph) => parseParagraph(paragraph, ctx, chain))

  let autofit: TextBody['autofit'] = 'none'
  const bodyPrNode = asXmlNode(bodyPr)
  if ('a:normAutofit' in bodyPrNode) autofit = 'shrink'
  else if ('a:spAutoFit' in bodyPrNode) autofit = 'resize'
  ...
  const naf = bodyPrNode['a:normAutofit']
  const nafAttr = (k: string): number | undefined => {
    const v = naf && typeof naf === 'object' ? asXmlNode(naf)[k] : undefined
  ...
  const vertRaw = attribute(bodyPrNode, '@_vert')
  ...
  return {
    paragraphs,
    anchor: attribute(bodyPrNode, '@_anchor')
      ? anchorMap[attribute(bodyPrNode, '@_anchor')!]
      : undefined,
    insets: {
      l: intOr(bodyPrNode['@_lIns'], 91440),
      t: intOr(bodyPrNode['@_tIns'], 45720),
      r: intOr(bodyPrNode['@_rIns'], 91440),
      b: intOr(bodyPrNode['@_bIns'], 45720),
    },
    autofit,
    ...(fontScale != null ? { fontScale } : {}),
    ...(lnSpcReduction != null ? { lnSpcReduction } : {}),
    wrap: bodyPrNode['@_wrap'] !== 'none',
    ...(vert ? { vert } : {}),
  }
}

/** <a:spcPct val="150000"/> → 150 (%). */
function spcPct(value: unknown): number | undefined {
  const v = attribute(child(value, 'a:spcPct'), '@_val')
  return v != null ? (parseInt(v, 10) || 0) / 1000 : undefined
}

/** <a:spcPts val="2400"/> → 24 (pt). */
function spcPts(value: unknown): number | undefined {
  const v = attribute(child(value, 'a:spcPts'), '@_val')
...
function parseParagraph(
  ...
  const level = pPr['@_lvl'] ? intOr(pPr['@_lvl'], 0) : undefined
  ...
  const runsRaw = xmlArray(p['a:r'])
  const runs: TextRun[] = runsRaw.map((r) => {
  ...
  const fldsRaw = xmlArray(p['a:fld'])
  ...
  else if (attribute(child(pPr, 'a:buChar'), '@_char') != null) {
    bullet = { type: 'char', char: decodeCharRefs(attribute(child(pPr, 'a:buChar'), '@_char')!) }
  } else if (pPr['a:buAutoNum']) {
    bullet = { type: 'number' }
    const numType = attribute(child(pPr, 'a:buAutoNum'), '@_type')
    if (numType) bullet.numType = numType
  }
  if (bullet && bullet.type !== 'none') {
    ...
    const bulletFont = attribute(child(pPr, 'a:buFont'), '@_typeface')
    if (bulletFont) bullet.font = bulletFont
    const bulletSize = attribute(child(pPr, 'a:buSzPct'), '@_val')
    if (bulletSize) {
      const v = parseInt(bulletSize, 10)
      if (Number.isFinite(v)) bullet.sizePct = v / 1000
    }
  }

  const marLRaw = pPr['@_marL'] != null ? intOr(pPr['@_marL'], 0) : undefined
  const indentRaw = pPr['@_indent'] != null ? intOr(pPr['@_indent'], 0) : undefined
  ...
  return {
    runs,
    align: attribute(pPr, '@_algn') ? alignMap[attribute(pPr, '@_algn')!] : dflt?.align,
    level,
    pPrExplicit,
...
function parseRun(r: XmlNode, ctx: ParseContext, dflt?: LevelTextStyle): TextRun {
  ...
  const text = decodeCharRefs(
    typeof rawT === 'string'
      ...
      : rawT == null
        ? ''
        : typeof rawT === 'object'
          ? String(asXmlNode(rawT)['#text'] ?? '')
          : String(rawT),
  )
  const hlink = child(rPr, 'a:hlinkClick')
  const hlinkId = attribute(hlink, '@_r:id')
  const hlinkTarget = hlinkId ? ctx.hlinkRels?.get(hlinkId) : undefined
  ...
  const colorFollowsTheme = color != null && !(fill && asXmlNode(fill)['a:srgbClr'])
  const colorInherited = color != null && !fill
  // Font: run explicit (incl. +mj/+mn theme refs) → inherited default → theme font
  const latinRaw = attribute(child(rPr, 'a:latin'), '@_typeface')
  const eaRaw = attribute(child(rPr, 'a:ea'), '@_typeface')
  const csRaw = attribute(child(rPr, 'a:cs'), '@_typeface')
  const latin = resolveFontRef(latinRaw, ctx.theme) ?? dflt?.latinFont
  const ea = resolveFontRef(eaRaw, ctx.theme) ?? dflt?.eaFont
  const cs = resolveFontRef(csRaw, ctx.theme) ?? dflt?.csFont
  ...
  const lnRaw = rPr['a:ln']
  const lnNode = asXmlNode(lnRaw)
  if (lnRaw && typeof lnRaw === 'object' && lnNode['a:solidFill']) {
    const lnColor = resolveColorNode(lnNode['a:solidFill'], ctx)
    if (lnColor) {
      const w = lnNode['@_w'] != null ? intOr(lnNode['@_w'], 9525) : 9525
  ...
  return {
    ...
    fontSize: rPr['@_sz'] ? intOr(rPr['@_sz'], 0) / 100 : dflt?.fontSize,
    ...(rPr['@_sz'] ? {} : { fontSizeImplicit: true }),
    ...(rPr['@_spc'] ? { letterSpacing: intOr(rPr['@_spc'], 0) / 100 } : {}),
    ...(rPr['@_baseline'] ? { baseline: intOr(rPr['@_baseline'], 0) / 1000 } : {}),
    fontFamily,
    color,
