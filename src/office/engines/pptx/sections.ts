...
export function setSections(opened: OpenedPptx, sections: SectionInfo[]): void {
  ...
  archive.entries.set(PRES_PATH, utf8Encode(next))
}
...
export function moveSlide(opened: OpenedPptx, fromIndex: number, toIndex: number): boolean {
  ...
  archive.entries.set(
    PRES_PATH,
    utf8Encode(pres.replace(m[0], `<p:sldIdLst>${tags.join('')}</p:sldIdLst>`)),
  )
...
export function moveSection(
  ...
  archive.entries.set(
    PRES_PATH,
    utf8Encode(pres.replace(m[0], `<p:sldIdLst>${newInner}</p:sldIdLst>`)),
  )
