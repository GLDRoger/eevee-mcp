...
import { decodeBase64, encodeBase64, utf8Decode, utf8Encode } from './bytes'
...
function collectPart(
  ...
  if (!bytes) return
  parts[partPath] = encodeBase64(bytes)
  ...
  if (!relsXml) return
  parts[relsPath] = encodeBase64(utf8Encode(relsXml))
...
function writeParts(
  ...
  for (const [sourcePath, base64] of Object.entries(parts)) {
    ...
    let relsXml = utf8Decode(decodeBase64(sourceRels))
    ...
    archive.entries.set(relsPathFor(targetPath), utf8Encode(relsXml))
  }
...
function ensureContentTypes(
  ...
  archive.entries.set(ctPath, utf8Encode(ct))
}
...
function partText(parts: Readonly<Record<string, string>>, path: string): string | null {
  ...
  return base64 == null ? null : utf8Decode(decodeBase64(base64))
}
