...
import type { DrawingInput, FormValueInput, MarkupType, MetadataInput, StampInput } from './ipc'
import type { PdfBrowserApi } from './browser-api'
import type { DriveFileHandle } from '../host/storage'

GlobalWorkerOptions.workerSrc = '/pdfjs/pdf.worker.min.mjs'

const documentLoadingTasks = new WeakMap<PDFDocumentProxy, PDFDocumentLoadingTask>()
...
const createAnnotationId = (prefix: 'd' | 'm'): string => `${prefix}${crypto.randomUUID()}`

const renderStampConfig = (
  config: StampConfig,
  pages: number[],
  sizes: PageSize[],
): StampInput[] =>
  buildStamps(
    pages.map((originalIndex, displayIndex) => ({
      ...
      pw: sizes[originalIndex]!.width,
      ph: sizes[originalIndex]!.height,
      displayNo: displayIndex + 1,
    })),
    config.wm,
    config.hf,
  )
...
export default function App({
  ...
  api: PdfBrowserApi
  initialHandle: DriveFileHandle | null
  onClose: () => void
}) {
  const { t } = useI18n()
  const [doc, setDoc] = useState<PDFDocumentProxy | null>(null)
  const [fileHandle, setFileHandle] = useState<DriveFileHandle | null>(initialHandle)
  ...
  const loadDocBytes = useCallback(async (data: Uint8Array) => {
    ...
    documentLoadingTasks.set(loaded, loadingTask)
    const all: PageSize[] = []
    const rots: number[] = []
    ...
    setUndoStack([])
    setRedoStack([])
    setReadOnly(passwordRef.current !== undefined)
  ...
  const loadDoc = useCallback(
    async (handle: DriveFileHandle) => loadDocBytes(new Uint8Array(await api.readFile(handle))),
    [api, loadDocBytes],
  )

  useEffect(
    () => () => {
      if (doc) void documentLoadingTasks.get(doc)?.destroy()
    },
    [doc],
  )

  const openHandle = useCallback(
    async (handle: DriveFileHandle) => {
      try {
        setFileHandle(handle)
        // A newly opened file starts outside the autosave gate
        savedOnceRef.current = false
        if (!(await loadDoc(handle))) return
        setStatus('ready')
      } catch (err) {
  ...
  // Browser navigation has no desktop close handshake. Warn before a dirty tab unloads.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      void getSearchIndex()?.then((idx) => {
        if (cancelled) return
        setSearchMatches(searchInIndex(idx, query))
        setSearchCur(0)
      })
    }, query ? 200 : 0)
    return () => {
  ...
  const editsPayload = () => ({
    markups: markups.map(({ pageIndex, type, color, quads }) => ({ pageIndex, type, color, quads })),
    drawings: drawings.map((d) => d.input),
    stamps: stampCfg ? renderStampConfig(stampCfg, visList, sizes) : [],
  ...
  const save = (autosave = false): Promise<boolean> => {
    if (!dirty || saveState === 'saving' || !fileHandle) return Promise.resolve(!dirty)
    // An explicit save opts this file into autosave
    if (!autosave) savedOnceRef.current = true
    const run = (async (): Promise<boolean> => {
      setSaveState('saving')
      const result = await api.save({ handle: fileHandle, ...editsPayload() })
      if (!result.ok) {
  ...
  /**
   * Save As: apply pending edits onto the source bytes and write only to targetName.
   * The original file stays untouched on disk and the edits stay pending in this tab.
   */
  const saveAsTo = async (targetName: string): Promise<boolean> => {
    ...
    const edits = flushed
      ...
      : editsPayload()
    setSaveState('saving')
    const result = await api.save({ handle: fileHandle, targetName, ...edits })
    if (!result.ok) {
      opFailed(result.error)
      return false
    }
  ...
  useAutosave(
    () =>
      savedOnceRef.current && dirty && saveState !== 'saving' && fileHandle !== null && !readOnly,
    () => void save(true),
  )
  ...
  const commitDrawing = (origIdx: number, input: DrawingInput) => {
    ...
    setDrawings((prev) => [
      ...prev,
      { id: createAnnotationId('d'), input: { ...input, pageIndex: origIdx } },
    ])
  }
  ...
  const stampPreview = useMemo(() => {
    ...
    for (const s of renderStampConfig(stampCfg, visList, sizes)) {
    ...
    return byPage
  }, [stampCfg, visList, rows, visibleRows, sizes])
  ...
  const insertPdf = (afterOrigIdx: number) =>
    flushThen(async () => {
      if (!fileHandle) return
      const result = await api.insertPdf({ handle: fileHandle, afterPageIndex: afterOrigIdx })
      if (!result.ok) {
        opFailed(result.error)
        return
      }
      if (!('canceled' in result)) await loadDoc(fileHandle)
    })
  ...
  const printDoc = () =>
    flushThen(async () => {
      ...
      setPrinting(true)
      try {
        const data = await api.readFile(fileHandle)
        ...
        <div className="pdf-placeholder">
          <form
            ...
            onSubmit={(e) => {
              ...
              setStatus('loading')
              if (fileHandle) void openHandle(fileHandle)
            }}
          >
      ...
      <div className="app">
        <div className="ribbon">
          <div className="ribbon-tabs">
            <span className="ribbon-file">{fileName || 'PDF editor'}</span>
            <span className="ribbon-tabs-spacer" />
            <button className="qa-btn" type="button" title="Close editor" onClick={onClose}>
              ×
            </button>
          </div>
        </div>
  ...
  return (
    <div className="app">
      <div className="ribbon">
        <div className="ribbon-tabs">
          <span className="pdf-file-tab-wrap">
              <button
                className={`pdf-file-tab ${fileMenuOpen ? 'open' : ''}`}
                type="button"
          ...
          <>
              ...
              <button
                ...
                disabled={!fileHandle || saveState === 'saving'}
                onClick={() => {
                  void api.requestName(fileName).then((name) => {
                    if (name) void saveAsTo(name)
                  })
                }}
              >
                ⧉
          ...
          <span className="ribbon-tabs-spacer" />
          <span className="ribbon-file" title={fileName}>
            {fileName}
          </span>
