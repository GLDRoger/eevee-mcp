...
export async function printPdf(doc: PDFDocumentProxy): Promise<void> {
  ...
  canvas.width = 0
  canvas.height = 0
  const officeRoot = document.querySelector<HTMLElement>('.office-pdf')
  if (!officeRoot) throw new Error('PDF print requires an .office-pdf root')
  officeRoot.appendChild(root)
  try {
