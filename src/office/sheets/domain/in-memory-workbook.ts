...
export class InMemoryWorkbookAdapter implements WorkbookAdapter {
  ...
  plan(input: unknown): ChangePlan {
    ...
    if (batch.baseRevision !== this.snapshot.revision) {
      throw new WorkbookConflictError('The workbook changed after the generated context was captured.')
    }
  ...
  /// a pending generated preview's CAS base must not be invalidated by them.
