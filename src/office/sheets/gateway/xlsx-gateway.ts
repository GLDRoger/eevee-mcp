...
import { resolveRelationshipTarget } from './xlsx-relationships'
...
export async function planCellEditsToXlsx(
  ...
  const sheetNames = new Set([
    ...
    ...formulaValues.map((state) => state.sheetName),
  ])
  ...
  for (const sheet of formulaValues) {
    ...
    if (worksheetXml === undefined) {
      throw new Error(`Formula values reference unknown sheet "${sheet.sheetName}".`)
    }
...
async function shiftAnchoredSheetParts(
  ...
  for (const { relId, kind } of parts) {
    ...
    const partPath = resolveRelationshipTarget(worksheetPath, target, kind)
...
async function resolveWorksheetPath(
  ...
  return resolveRelationshipTarget('xl/workbook.xml', targetMatch, 'worksheet')
}
