...
const EMPTY_RELS =
  ...
  '</Relationships>'
const EXTERNAL_HYPERLINK_PROTOCOLS = new Set(['https:', 'http:', 'mailto:', 'tel:'])

function validatedExternalTarget(target: string): string {
  const candidate = target.trim()
  let protocol: string
  try {
    protocol = new URL(candidate).protocol
  } catch {
    throw new HyperlinkEditError(
      'External spreadsheet hyperlinks must be valid http, https, mailto, or tel URLs.',
    )
  }
  if (!EXTERNAL_HYPERLINK_PROTOCOLS.has(protocol)) {
    throw new HyperlinkEditError(
      'External spreadsheet hyperlinks must use http, https, mailto, or tel.',
    )
  }
  return candidate
}
...
export function applyHyperlinkEdits(
  ...
  for (const edit of edits) {
    ...
    const element = edit.target.startsWith('#')
      ...
      : `<hyperlink ref="${ref}" r:id="${allocateRel(validatedExternalTarget(edit.target))}"/>`
