...
interface ExcelShellProps {
  ...
  /// Left side of the status bar (ready / streaming / generated progress messages).
  ...
  /// shares the generated panel's onUndo above.
...
export function ExcelShell({
  ...
  const renderedTab = !selectedChart && activeTab === 'Chart Design' ? 'Home' : activeTab
  ...
  return (
    <main className="app-shell">
      <header className="excel-header">
        ...
        >
          <span className="sheets-file-tab-wrap">
              <button
                type="button"
                className={`sheets-file-tab ${fileOpen ? 'open' : ''}`}
                onClick={() => setFileOpen((open) => !open)}
              >
                File
              </button>
              {fileOpen ? (
                <span className="sheets-file-menu">
                  <>
                    <button
                      type="button"
                      disabled={!canSave}
          ...
          {visibleTabs.map((tab) => (
            <button
              className={`${tab === renderedTab ? 'active' : ''} ${tab === 'Chart Design' ? 'contextual' : ''}`}
              key={tab}
