...
function enhanceCfPanel(runtime: UniverRuntime): () => void {
  ...
  const augment = (): void => {
    // Univer creates the locale service before its initial locale is ready.
    // MutationObserver can fire in that gap during workbench startup; the next
    // panel mutation will retry after initialization.
    let label: string
    try {
      label = locale.t('sheets-conditional-formatting-ui.ruleType.formula')
    } catch {
      return
    }
