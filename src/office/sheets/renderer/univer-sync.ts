...
export function applyHyperlinkPlan(
  state: LazyWorkbookState,
...
export function applyConditionalFormatPlan(
  worksheet: UniverWorksheet,
  ...
  let styled = buildRuleHighlight(builder, rule)
...
export function applyDataValidationPlan(
...
/// Loads an generated-requested range before its cells are read from Univer. Normal
/// viewport loading is intentionally fire-and-retry; generated reads instead wait
...
/// Shared landing path for column filter criteria: the generated op
/// `set_filter_criteria` and the Advanced Filter dialog both come through
/// here, so manual and generated edits hit the same facade command (and journal
...
/// Reads a cell's current content for generated previews and drift checks.
export function lazyCellReader(worksheet: UniverWorksheet): (address: string) => CellState {
  return (address) => {
    const range = worksheet.getRange(address)
    const formula = range.getFormula()
    const value = range.getValue() ?? null
    // Formula cells also carry their computed value (the generated needs to see results
...
/// Mirrors the BeforeSheetEditStart streaming guard for generated-planned cells.
