/**
 * Chart/shape/image insertion (ribbon and generated ops). Extracted from App.tsx;
...
/// adapter add_chart op as the generated path, so the chart lives in the snapshot
...
/// generated edit_chart → renderer edit data. Series ranges are read from the
/// sheet here so the file cache and the on-screen render update together.
export async function buildChartEdit(
  ctx: VisualActionContext,
  state: LazyWorkbookState,
...
/// generated add_image: same visual pipeline as the picker-based insert, sized
/// from the measured natural dimensions.
export function insertImageVisual(
  ctx: VisualActionContext,
  runtime: UniverRuntime,
  state: LazyWorkbookState,
...
/// generated edit_shape: in-place journal update of a session-added shape,
...
/// generated add_chart: same visual-add pipeline as the ribbon's Insert Chart,
...
export async function insertChartVisual(
  ctx: VisualActionContext,
  runtime: UniverRuntime,
  state: LazyWorkbookState,
...
export function insertShapeVisual(
  ctx: VisualActionContext,
  runtime: UniverRuntime,
  state: LazyWorkbookState,
