 ...
 *
 * generated DSL operations (pivot / table / table-column adds) and pivot dialog
...
export function applyTableAdd(
  ...
  void worksheet.addTable(
    ...
    `table-${state.editJournal.tableAdds.length + 1}-${Date.now().toString(36)}`,
  )
...
/// generated add_table_row: inserts one or more rows into a session-added table.
/// The row index is 1-based within the data region (header = row 0).
export function applyTableRowAdd(
  runtime: UniverRuntime,
  state: LazyWorkbookState,
...
/// generated delete_table_row: removes one or more rows from a session-added table.
export function applyTableRowDelete(
  runtime: UniverRuntime,
  state: LazyWorkbookState,
...
/// generated delete_table_column: removes one or more columns from a session-added table.
export function applyTableColumnDelete(
  runtime: UniverRuntime,
  state: LazyWorkbookState,
...
export function applyTableColumnAdd(
  runtime: UniverRuntime,
  state: LazyWorkbookState,
