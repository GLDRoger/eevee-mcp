...
const THUMBS_W_KEY = 'eevee.slides.thumbsWidth'
...
export function App({ onClose }: { onClose: () => void }) {
  ...
  // Re-clamp when the window shrinks (max is 40% of the window).
  useEffect(() => {
  ...
  const [autoSave, setAutoSave] = useState(
    () => localStorage.getItem('eevee.slides.autoSave') === '1',
  )
  useEffect(() => {
    localStorage.setItem('eevee.slides.autoSave', autoSave ? '1' : '0')
  ...
  const rawFit = useCallback(
    (s?: RenderSlide) => {
      ...
      const availW =
        (el?.clientWidth ?? window.innerWidth - (showThumbs ? thumbsW : 0)) - 56
  ...
  // Text dragged in plain DOM focuses body; canvas shape selection stays collapsed.
  ...
  // Files are pushed open by the host; with no
  // pending file the window lands on a fresh blank deck. StrictMode runs the mount effect twice,
  // but the pending queue can only be consumed once, so the consume Promise is stored in a
  // shared ref and its result is processed only once.
  useEffect(() => {
    if (bootedFileRef.current === '__file__') return
    bootedFileRef.current = '__file__'
  ...
  useEffect(() => {
    if (!path) return
    try {
      const raw = localStorage.getItem(`eevee.slides.guides:${path}`)
      setGuides(
        raw
          ? JSON.parse(raw)
          : [
  ...
  useEffect(() => {
    if (!path) return
    try {
      localStorage.setItem(`eevee.slides.guides:${path}`, JSON.stringify(guides))
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(`eevee.slides.customShows:${path}`)
  ...
  return (
    <div className="app">
      <Ribbon
        ...
        onDownload={() => void fileActions.download(ctxRef.current)}
