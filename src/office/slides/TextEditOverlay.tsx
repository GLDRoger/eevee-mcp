...
import type { GlyphRun, ShapeRenderNode, TextLine } from '@/office/engines/pptx-render'
  ...
  groupLinesToParagraphs(lines).forEach((paraLines, pi) => {
    ...
    for (const { run, srcRun, text } of editorParaRuns(paraLines)) {
      if (run) {
        ...
        const link = decodeLinkTarget(run.link)
        const span = reuse ? prev! : document.createElement(link ? 'a' : 'span')
        if (!reuse) {
          span.dataset.runContainer = 'true'
          if (src) span.dataset.srcRun = src
          if (link) span.setAttribute('href', encodeLinkTarget(link))
          ...
          else if (link) span.style.textDecoration = 'none' // suppress the UA <a> underline
