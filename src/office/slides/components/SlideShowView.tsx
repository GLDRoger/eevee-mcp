...
import type { RenderNode, RenderSlide, ShapeRenderNode } from '@/office/engines/pptx-render'
...
export function SlideShowView({
  ...
  const [rehearseNow, setRehearseNow] = useState(() => Date.now())
  useEffect(() => {
    if (!rehearseMode) return
    rehearseRef.current = startRehearse(slides.length, order[pos] ?? startAt, Date.now())
    const h = window.setInterval(() => setRehearseNow(Date.now()), 500)
  ...
  }, [allAnims, pos, order, player.load])
  ...
  }, [ended, pos, order.length, goTo, player.advance])
  ...
  const sinceEntered = rehearse ? Math.max(0, rehearseNow - rehearse.enteredAt) : 0
...
function ShowMediaLayer({
  ...
  const nodes = slide.nodes.filter(
    (n): n is import('@/office/engines/pptx-render').PictureRenderNode =>
      n.type === 'picture' && !!(n as import('@/office/engines/pptx-render').PictureRenderNode).media,
  )
