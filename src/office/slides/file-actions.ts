...
/** Browser download of the current deck. Used in session mode (host owns Save). */
export async function download(ctx: ActionCtx): Promise<void> {
  await flushActiveEdit(ctx)
  await ctx.flushNotes()
  try {
    const bytes = await window.slidesApi.exportBytes()
    const rawName = ctx.path?.split('/').pop() || t('appUntitledPresentation')
    const name = rawName.toLowerCase().endsWith('.pptx') ? rawName : `${rawName}.pptx`
    ...
    ctx.setStatus(t('appStatusSaveFailed', { error: String(err) }))
  }
}
