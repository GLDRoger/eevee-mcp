...
import { substituteFamily } from './fonts'
...
export function displayFontFamily(name: string): string {
  const drawn = substituteFamily(name)
  const stack = FONT_STACK[drawn.normalize('NFKC').toLowerCase()]
  if (stack) return stack
  if (drawn !== name) return `'${drawn}', sans-serif`
