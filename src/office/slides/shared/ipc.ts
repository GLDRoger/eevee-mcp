/**
 * Slides host and renderer contract for open, save, edit, and presentation controls.
 *
 ...
 */
import type { RenderSlide } from '@/office/engines/pptx-render'
import type { SlideComment, SectionInfo } from '@/office/engines/pptx'

export type { SlideComment, SectionInfo } from '@/office/engines/pptx'
...
export interface SlidesApi {
  ...
  /** Open in-memory pptx bytes (applet session / host-owned draft). Does not touch the drive. */
  openFromBytes: (bytes: Uint8Array, path: string, fitWidthPx: number) => Promise<OpenResult>
  /** Current deck as pptx bytes (host publish / Download). Does not write the drive. */
